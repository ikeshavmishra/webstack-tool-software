...
// ────────────────────────────────────────────
// Widevine (Chrome DLL on Windows)
// ────────────────────────────────────────────
//
// 1. Find Chrome's Widevine DLL, e.g. something like:
//    C:\Program Files (x86)\Google\Chrome\Application\123.0.0.0\WidevineCdm\_platform_specific\win_x64\widevinecdm.dll
// 2. Put that full path below (escape backslashes: \\)
// 3. Put the correct Widevine version string below
// 4. Set USE_CHROME_WIDEVINE = true
//
// NOTE: If path/version are wrong, plugin just won't load (no crash).
//

const USE_CHROME_WIDEVINE = true; // set to true after updating path + version

const WIDEVINE_CDM_PATH =
  'C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\143.0.3650.80\\WidevineCdm\\_platform_specific\\win_x64\\widevinecdm.dll';

const WIDEVINE_CDM_VERSION = '4.10.2891.122';

if (USE_CHROME_WIDEVINE) {
  app.commandLine.appendSwitch('widevine-cdm-path', WIDEVINE_CDM_PATH);
  app.commandLine.appendSwitch('widevine-cdm-version', WIDEVINE_CDM_VERSION);
}
...
function logWidevineStatus(webContents, label = '') {
  ...
  webContents.executeJavaScript(`
    (async () => {
      ...
      try {
        ...
        emeOk = false;
        emeError = { name: e.name, message: e.message };
      }

      return { plugins, emeOk, emeError };
    })();
  `, true).then(result => {
    console.log('──────── Widevine check', label, '────────');
    console.log('Plugins:', result.plugins);
    console.log('EME Widevine access OK:', result.emeOk);
    console.log('EME error:', result.emeError);
    console.log('───────────────────────────────────────');
  }).catch(err => {
    console.log('Widevine check error', label, err);
  });
}
...
function wireTabEvents(tab) {
  ...
  wc.on('did-finish-load', () => {
    ...
    if (!tab._widevineChecked) {
    tab._widevineChecked = true;
    logWidevineStatus(wc, `tab ${tab.id} (${tab.url})`);
    }
